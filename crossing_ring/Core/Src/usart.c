...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
  ...
  {
    // HAL_UART_Transmit(&huart2, (uint8_t*)&rx_data, 1, 0xffff); // loop test, normal use requires annotation. 
    
    write_1byte(&ringbuffer, rx_data);
    
    /* ? \r\n ????????? */
    if(break_flag && rx_data == 0x0a)
    {
      break_flag = 0;
      rx_complt = 1;
    }
    if(rx_data==0x0d)break_flag = 1;
    

    /* ? 0 ????????? */
    // if(rx_data==0x00)rx_complt=1;
